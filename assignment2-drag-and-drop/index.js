...
for (var i = 1; i <= users.length; i++) {
	...
	elem.setAttribute('ondragend', 'ondragend_handler(event)');
	elem.setAttribute('ondrag', 'drag_handler(event)');
	...
	elem.innerText = users[i - 1].first_name + ' ' + users[i - 1].last_name;

	elements.appendChild(elem);
	if (i % 3 == 1) {
		col1Element.innerHTML =
			col1Element.innerHTML +
			'<div id="s' +
			i +
